...
def _agg_iso_projects_to_counties(iso_df: pd.DataFrame) -> pd.DataFrame:
    ...
    projects['renew_fossil'] = projects.loc[:,
                                            'resource_clean'].map(renewable_fossil_dict)
    ...
    projects['breakout_fuels'] = projects.loc[:,
                                              'resource_clean'].replace({'Natural Gas': 'Gas'})
    projects.loc[:, 'breakout_fuels'] = projects.loc[:, 'breakout_fuels'].str.lower()
    fuels_to_breakout = {'solar', 'onshore wind',
                         'offshore wind', 'gas', 'battery storage'}
...
def _agg_pudl_to_counties(pudl_df: pd.DataFrame) -> pd.DataFrame:
    ...
    plants['renew_fossil'] = plants.loc[:,
                                        'fuel_type_code_pudl'].map(renewable_fossil_dict)
...
def _flatten_multiindex(multi_index: pd.MultiIndex, prefix: str) -> pd.Index:
    flattened = ['_'.join([levels[0], prefix, *levels[1:]]).replace(' ', '_')
...
def make_county_data_mart_table(engine: Optional[sa.engine.Engine] = None) -> pd.DataFrame:
    ...
    out = out.merge(all_states[['state_abbrev', 'state_id_fips']],
                    on='state_id_fips', how='left', validate='m:1')
    out = out.merge(all_counties[['county_name', 'county_id_fips']],
