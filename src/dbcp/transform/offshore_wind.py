...
def _create_association_table(
    ...
    association_table = exploded[[pk, array_col]].copy()
    association_table[array_col] = pd.to_numeric(
        association_table[array_col], errors="raise"
    ).astype("Int64")
    association_table = association_table.rename(columns={array_col: array_col_rename})
